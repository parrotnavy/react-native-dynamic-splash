...
class AppDelegate {}`;
  ...
  describe("iOS - Objective-C transformations", () => {
    ...
    const injectObjCShow = (contents: string): string => {
      ...
      if (contents.includes(showTag)) return contents;
      const regex = /(-\s*\(BOOL\)\s*application\s*:\s*\(UIApplication\s*\*\s*\)application\s*didFinishLaunchingWithOptions\s*:\s*\(NSDictionary\s*\*\s*\)launchOptions\s*\{)/;
...
@end`;
  ...
  describe("Android - Kotlin transformations", () => {
    const injectKotlinImport = (contents: string): string => {
      const importTag = "import com.reactnativedynamicsplash.DynamicSplashNativeModule";
      if (contents.includes(importTag)) return contents;
    ...
    const injectKotlinShow = (contents: string): string => {
      ...
      if (contents.includes("super.onCreate(savedInstanceState)")) {
        return contents.replace("super.onCreate(savedInstanceState)", `super.onCreate(savedInstanceState)\n    ${showTag}`);
      }
      return contents;
    };
...
class MainActivity : ReactActivity()`;
      ...
      expect(result).toContain("import com.reactnativedynamicsplash.DynamicSplashNativeModule");
    });
...
class MainActivity : ReactActivity()`;
      ...
      const matches = result.match(/import com\.reactnativedynamicsplash\.DynamicSplashNativeModule/g);
      expect(matches).toHaveLength(1);
    });
