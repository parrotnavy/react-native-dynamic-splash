...
describe("SplashStorage", () => {
	...
	describe("constructor", () => {
		it("uses default storage key when not specified", () => {
			...
			expect(
				NativeModules.DynamicSplashNative.setStorageKey,
			).toHaveBeenCalledWith("DYNAMIC_SPLASH_META_V1");
		});
		...
		it("uses custom storage key when specified", () => {
			...
			expect(
				NativeModules.DynamicSplashNative.setStorageKey,
			).toHaveBeenCalledWith("CUSTOM_KEY");
		});
		...
		it("uses getStringSync when available", () => {
			...
			(
				NativeModules.DynamicSplashStorage.getStringSync as jest.Mock
			).mockReturnValue(JSON.stringify(storedMeta));

			const options = createOptions();
			const storage = new SplashStorage(options);

			expect(
				NativeModules.DynamicSplashStorage.getStringSync,
			).toHaveBeenCalled();
	...
	describe("setMeta", () => {
		...
		it("uses custom storage key for persistence", () => {
			...
			const storage = new SplashStorage(
				createOptions({ storageKey: "CUSTOM_KEY" }),
			);
	...
	describe("ready", () => {
		...
		it("resolves with false on timeout", async () => {
			...
			// Restore
			(NativeModules.DynamicSplashStorage as any).getStringSync =
				originalGetStringSync;
		});
	});
