...
describe("sync", () => {
  ...
  describe("performBackgroundUpdate", () => {
    ...
    it("saves ERROR when no configs within time window", async () => {
      const farFuture = new Date(now + 1000 * 60 * 60 * 24 * 365 * 10).toISOString();
      const farFuturePlus = new Date(now + 1000 * 60 * 60 * 24 * 365 * 11).toISOString();
      const configs: SplashConfig[] = [
  ...
  describe("processSplashConfig", () => {
    ...
    it("selects config by weight from array", async () => {
      ...
      const highWeightCount = selectedNames.filter((n) => n === "high-weight").length;
