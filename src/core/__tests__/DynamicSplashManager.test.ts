...
describe("DynamicSplashManagerImpl", () => {
  ...
  describe("mount()", () => {
    ...
    it("logs when logger is provided", async () => {
      ...
      expect(mockLogger).toHaveBeenCalledWith(expect.stringContaining("Mount called"));
    });
  ...
  describe("isVisible()", () => {
    it("returns true when native isShowing returns true", async () => {
      (NativeModules.DynamicSplashNative.isShowing as jest.Mock).mockResolvedValue(
        true
      );
    ...
    it("returns false when native isShowing throws", async () => {
      (NativeModules.DynamicSplashNative.isShowing as jest.Mock).mockRejectedValue(
