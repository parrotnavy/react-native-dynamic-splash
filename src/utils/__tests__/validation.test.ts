import { validateSplashConfig, isWithinTimeWindow } from "../validation";
...
describe("validateSplashConfig", () => {
  ...
  describe("imageUrl validation", () => {
    it("rejects non-http URL", () => {
      const config = { ...validConfig, imageUrl: "ftp://example.com/image.png" };
      expect(validateSplashConfig(config)).toBe(false);
    });
    ...
    it("accepts https URL", () => {
      const config = { ...validConfig, imageUrl: "https://example.com/image.png" };
      expect(validateSplashConfig(config)).toBe(true);
    });

    it("accepts http URL", () => {
      const config = { ...validConfig, imageUrl: "http://example.com/image.png" };
      expect(validateSplashConfig(config)).toBe(true);
    });
  });
  ...
  describe("weight validation", () => {
    it("accepts undefined weight", () => {
      const { weight, ...config } = { ...validConfig, weight: 1 };
      expect(validateSplashConfig(validConfig)).toBe(true);
    });
